...
def test_bayesian_result_changes_when_sample_points_change():
    def _sample_driven_fake_r(payload):
        samples = [e for e in (payload.get("episode_events") or []) if str(e.get("event_type", "")).lower() == "sample"]
        values = [float(str(e.get("level_mg_l")).replace(",", ".")) for e in samples]
        avg = sum(values) / max(len(values), 1)
        cl = round(max(0.5, avg / 5.0), 4)
        vd = round(max(10.0, avg * 2.0), 4)
        curve = [avg, avg * 0.7]
        return {
            "status": "ok",
            "engine": "R_Bayesian",
            "model_key": payload.get("selected_model_key") or "goti_2018",
            "auc24": avg * 10,
            "auc_mic": avg * 10,
            "predicted_peak": max(curve),
            "predicted_trough": min(curve),
            "posterior_cl_l_h": cl,
            "posterior_vd_l": vd,
            "curve": {"x": [0, 1], "y": curve},
            "observed": {"x": [2, 11], "y": values},
            "dose_events": [],
            "warnings": [],
            "errors": [],
            "debug": {},
    ...
    with patch("tdm_platform.pk.vancomycin_engine.run_r_engine", side_effect=_sample_driven_fake_r):
        result_a = calculate(
            VancomycinInputs(
                sex="férfi",
                age=60,
                ...
                method="Bayesian",
                episode_events=[
                    {"event_type": "sample", "time_h": 2, "level_mg_l": 30},
                    {"event_type": "sample", "time_h": 11, "level_mg_l": 10},
                ],
            )
        )
        result_b = calculate(
            VancomycinInputs(
                ...
                episode_events=[
                    ...
                    {"event_type": "sample", "time_h": 11, "level_mg_l": 20},
                ],
            )
        )
    assert result_a["cl_l_h"] != result_b["cl_l_h"] or result_a["vd_l"] != result_b["vd_l"] or result_a["plot"]["single_model"]["pred_y"] != result_b["plot"]["single_model"]["pred_y"]
