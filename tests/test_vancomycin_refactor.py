...
def test_mic_and_creatinine_events_allow_empty_time_without_misleading_warning():
    payload = {
        "sex": "férfi",
        "age": 60,
        "height_cm": 175,
        ...
        "episode_events": [
            ...
            {"event_type": "sample", "time_h": "10", "level_mg_l": "10"},
            {"event_type": "mic_result", "time_h": "", "mic": "1"},
            {"event_type": "creatinine", "time_h": "", "creatinine": "100"},
        ],
    }
    _, summary = build_simple_episode(payload)
    assert not any("hibás time_h" in msg and ("mic_result" in msg or "creatinine" in msg) for msg in summary["validation_warnings"])
...
def test_classical_curve_keeps_negative_loading_event_and_stable_scale():
    ...
    y_vals = single.get("pred_y", [0.0])
    assert max(y_vals) < 1e4
    assert all(v < 1e6 for v in y_vals)
